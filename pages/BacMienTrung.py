...
def Title():
    ...
    with col1:
        st.write("")
    with col2:
        st.markdown("<h1 style='text-align: center; color: #B799FF;'>BAC MIEN TRUNG</h1>", unsafe_allow_html=True)
...
def main():
    # Config trang -----------------------------------------
    st.set_page_config(
        page_title="Bac Mien Trung",
        page_icon="📊",
        layout="wide"
    )
    Title()
    st.markdown("# Tài chính")
    df_bac_mien_trung = pd.read_excel("data/config/Config.xlsx",sheet_name="Bắc miền trung ",usecols="C:P", skiprows=11, nrows=12)
