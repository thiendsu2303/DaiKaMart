...
def Title():
    ...
    with col1:
        st.write("")
    with col2:
        st.markdown("<h1 style='text-align: center; color: #B799FF;'>TAY NGUYEN</h1>", unsafe_allow_html=True)
...
def main():
    # Config trang -----------------------------------------
    st.set_page_config(
        page_title="Tay Nguyen",
        page_icon="📊",
        layout="wide"
    )
    Title()
    st.markdown("## Tài chính")
    df_bac_mien_trung = pd.read_excel("data/config/Config.xlsx",sheet_name="Tây nguyên",usecols="C:P", skiprows=8, nrows=12)
